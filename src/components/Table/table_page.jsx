...
export default function EnhancedTable(props) {
    ...
    let $table = props.onRef || { noChecked:false,noSort:false };
    ...
    let rows = props.rows.records;
    let currentPage = props.rows.pageNum - 1;
    let currentPageCount = props.rows.pageCount;
    let perPage = props.rows.pageSize;
    //
    const [order, setOrder] = React.useState('asc');
    const [orderBy, setOrderBy] = React.useState('calories');// TODO
    const [page, setPage] = React.useState(currentPage);
    const [selected, setSelected] = React.useState(props.rows.selected);
    const [dense, setDense] = React.useState(Boolean(tableDense));
    const [rowsPerPage, setRowsPerPage] = React.useState(perPage);
    React.useEffect(()=>{
        // setSelected(props.rows.selected)
    })
    // parent-data
    $table.onDense = (checked) => {
        setDense(checked);
    }
    $table.onRefresh = (pagination) => { // TODO : React.useEffect()
        setPage(pagination.pageNum-1);
        setRowsPerPage(pagination.pageSize);
        setSelected(pagination.selected);
    }
    ...
    const handleClick = (event, name,row) => {
        ...
        let arr = [...selected];
        if (selectedIndex === -1) {
            arr.push(row[tableKey]);
        }else{
            arr.splice(selectedIndex,1)
        }
        //
        if(event && event.target.closest('.auto_event')){
            setSelected(arr);
            handle('selected',arr);
        }else{
            console.log('seelct',name)
            handle('detail',[name]);
        }
    };
                                ...
                                <TableBody>
                                    {
                                        rows.map((row, index) => {
                                            ...
                                            return (
                                                <TableRow
                                                    hover
                                                    onClick={event => handleClick(event, row[tableKey],row)}
                                                    role="checkbox"
                                                ...
                                                >
                                                    {
                                                        $table.noChecked
                                                            ...
                                                            : <TableCell padding="checkbox"><Checkbox className="auto_event" checked={isItemSelected} inputProps={{'aria-labelledby': labelId }} /></TableCell>
                                                    }
                                                    {
                                                        columns.map((v,i)=>{
                                                            let text = ''
