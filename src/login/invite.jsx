...
class Index extends Component {
    ...
    initCode(){
        ...
        if(!pm.code){
            ...
            History.push(`/login?from=${encodeURIComponent('/invite?group='+pm.group+'&user='+pm.user+'&code='+this.code)}`)
        }
    }
    ...
    clickInvite(){
        ...
        return ()=>{
            Service.inviteUserToGroup(code,(res)=>{
                if(res){
    ...
    render(){
        ...
        return (
            <div className={`flex-center ${classes.root}`} >
                <div>
                    <Typography variant="h5" gutterBottom>Invite you to <b>{params.group}</b> from {params.user}</Typography>
