...
class Index extends Component{
  ...
  componentWillMount(){
    ...
    this.$service = {
      ...
      dense:true,
      columns:[
        { id: 'name', disablePadding: true, label: '外部名称' },
        { id: 'loadbalance', disablePadding: true, label: '负载均衡器',style:{width:'180px'} },
        { id: 'clusterIp', disablePadding: true, label: '集群IP' },
        { id: 'port', disablePadding: true, label: '节点端口',render(row){
          let res = []
          row.servicePort.map((v)=>{
            res.push(v.port+'; ')
          })
          return res;
        }},
        { id: 'inpress', disablePadding: true, label: '入站' },
        { id: '' }
      ],
      event:(name,data)=>{
        switch (name){
  ...
  loadInfo(){
    ...
    Service.serviceMap({ServiceGroup: p._group,ServiceName: p._name},function(res){
      Service.serviceInfoList({serviceId:res.id},function(result){
  ...
  htmlService(){
    ...
    const { dense,serviceList } = this.state;
    return (
  ...
  render(){
    ...
    return (
        <>
        <Layout
            crumbList={[{text:params._group,url:`/group/${params._group}`},{text:params._name,url:`/summary/${params._group}/${params._name}/${params._memo}`},{text:'Services'}]}
            menuT={<MenuLayout params={params} />}
            contentT={sc.htmlService()}
        />
        <Drawer
            html={
              <>
